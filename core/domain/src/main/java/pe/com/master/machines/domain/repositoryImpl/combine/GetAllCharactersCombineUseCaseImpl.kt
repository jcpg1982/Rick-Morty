...
class GetAllCharactersCombineUseCaseImpl @Inject constructor(
    ...
    override fun invoke(
        ...
        val localFlow = storyCharacterLocalDataRepository.getCharactersByPage(PAGE_SIZE, offset)
        return if (!isHaveInternet) {
                        ...
                        if (local.data.isEmpty()) {
                            ...
                            Resource.Success(
                                ResponseAllCharacter(
                                    Info(pages = totalPages), local.data
                                )
                            )
                        }
                    }
            ...
            val remoteFlow = storyCharacterRemoteDataRepository.getLoadAllCharacters(page)
