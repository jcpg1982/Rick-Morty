...
class ViewEpisodesViewmodel @Inject constructor(
    ...
    private val _getStoryCharactersState: MutableStateFlow<ViewEpisodesState> =
        ...
        viewModelScope.launch {
            getEpisodesByIdsRemoteUseCase.invoke(ids)
                ...
                .onStart {
                    ...
                    _getStoryCharactersState.update { ViewEpisodesState.Loading }
                }
                .catch { e ->
                    Log.i(TAG, "getObtainEpisodes catch ${e.message}")
                    _getStoryCharactersState.update { ViewEpisodesState.Error(e) }
                }
                .collect { res ->
                    when (res) {
                        is Resource.Error -> {
                            Log.i(TAG, "getObtainEpisodes collect error ${res.messageError}")
                            _getStoryCharactersState.update { ViewEpisodesState.Error(Throwable(res.messageError)) }
                        }

                        is Resource.Success -> {
                            Log.i(TAG, "getObtainEpisodes collect Success ${res.data}")
                            _getStoryCharactersState.update { ViewEpisodesState.Success(res.data) }
                        }
                    }
                }
        }
